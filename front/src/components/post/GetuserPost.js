...
class GetuserPost extends Component {
  ...
  fetchPosts = () => {
    ...
    axios
      .get(`http://localhost:8080/users/${userId}/post`)
      .then((response) => {
  ...
  fetchComments = (postId) => {
    axios
      .get(`http://localhost:8080/users/${postId}/comment`)
      .then((response) => {
  ...
  handleDeleteComment = (commentId, postId) => {
    ...
    axios
      .delete(`http://localhost:8080/users/post/comment/delete/${commentId}`)
      .then((response) => {
  ...
  handleSubmitEditComment = async (postId) => {
    ...
    try {
      const response = await axios.put(
        `http://localhost:8080/users/post/comment/edit/${editingCommentId}`,
  ...
  fetchlikes = (postId) => {
    axios
      .get(`http://localhost:8080/users/post/${postId}/like/count`)
      .then((response) => {
        this.setState((prevState) => ({
          postlike: {
            ...prevState.postlike,
  ...
  handleLike = async (postId) => {
    ...
    try {
      const response = await axios.post(
        "http://localhost:8080/users/post/like",
        sendlikes,
        {
          headers: {
            "Content-Type": "application/json",
  ...
  handleSubmitComment = async (postId) => {
    ...
    try {
      const response = await axios.post(
        "http://localhost:8080/users/post/comment",
        sendcomment,
        {
          headers: {
            "Content-Type": "application/json",
  ...
  handleDelete = (postId) => {
    ...
    axios
      .delete(`http://localhost:8080/users/post/delete/${postId}`)
      .then((response) => {
        if (response.status === 200) {
             ...
             <CardContent>
               ...
               {totalImages > 0 && (
                 <div className="post-card-inner">
                   <img
                     src={`http://localhost:8080/uploads/${post.imagePaths[currentImageIndex]}`}
                     alt="Post"
